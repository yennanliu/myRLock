...
public enum LockTimeoutStrategy implements LockTimeoutHandler {
  ...
  FAIL_FAST {
    ...
    public void handle(LockInfo lockInfo, Lock lock) {
      String errorMsg =
          String.format(
              "Failed to acquire Lock(%s) with timeout(%ds)",
              lockInfo.getName(), lockInfo.getWaitTime());
    ...
    public void handle(LockInfo lockInfo, Lock lock) {
      ...
      while (!lock.acquire()) {
        if (interval > DEFAULT_MAX_INTERVAL) {
          String errorMsg =
              String.format(
                  "Failed to acquire Lock(%s) after too many times, this may be because deadlock occurred.",
                  lockInfo.getName());
          throw new MyRlockTimeoutException(errorMsg);
        }
